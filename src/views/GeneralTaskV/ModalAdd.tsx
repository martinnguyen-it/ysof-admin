...
} from '@/apis/generalTask/useMutationGeneralTask'
...
const ModalAdd: FC<IProps> = ({ open, setOpen }) => {
  ...
  useEffect(() => {
    if (open && open?.item) {
      setQuillValue(open.item.description)
    } else {
      setQuillValue('')
    }
  }, [open])
  ...
  const { data: documents, isLoading: loadingGetDocuments } =
    useGetListDocuments({ search: searchDebounce, season })
  ...
  const { mutate: mutateCreate, isPending: isPendingCreate } =
    useCreateGeneralTask(onSuccess)
  const { mutate: mutateUpdate, isPending: isPendingUpdate } =
    useUpdateGeneralTask(onSuccess)
      ...
      if (open?.item) {
        mutateUpdate({
          ...
          data: {
            ...data,
            start_at: startAt,
            end_at: endAt && endAt !== 'Invalid Date' ? endAt : undefined,
          },
        })
      } else {
        mutateCreate({
  ...
  useEffect(() => {
    if (open?.item)
      form.setFieldsValue({
        ...
        date_end_at: open.item.end_at
          ? dayjs(open.item.end_at, 'YYYY-MM-DD')
          : undefined,
      })

...
import { createGeneralTask, deleteGeneralTask, updateGeneralTask } from '@src/services/generalTask'
...
export const useUpdateGeneralTask = (onSuccess: () => void) =>
  useMutation({
    mutationFn: (payload: { id: string; data: IUpdateGeneralTask }) => updateGeneralTask(payload.id, payload.data),
    onSuccess,
    onError: (error: Error) => {
      const { message } = error
      toast.error(message)

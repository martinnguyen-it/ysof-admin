...
const FormInput = React.forwardRef<HTMLInputElement, Props>(({ label, error, className, ...rest }, ref) => {
  ...
  return (
    <div className={`flex w-full flex-col space-y-[6px] ${className}`}>
      ...
      <div className='flex w-full rounded-md border border-[#EBEBEB] px-[13px] py-[10px] text-xs text-cinder md:py-[14px] md:text-sm'>
        <input {...rest} type={type} ref={ref} id={label.replace(' ', '')} className='w-full outline-none focus:outline-none bg-white' />
